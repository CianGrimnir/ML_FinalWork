...
for q in qrange:
    ...
    history = model.fit(XX[train], yy[train], validation_data=(XX[test], yy[test]),
                        epochs=5000, batch_size=100, verbose=0, callbacks=[es])
...
# BASELINE MODEL - seasonal naive forecast model
...
print(f'r2 score for baseline model: {r2_score(test_y, predictions)}')
